...
fio_auto_test_multi_process()
{
	...
	mode_index="0"
	test_block_size=("1k" "2k" "4K" "8K" "16K" "32K" "64K" "128K" "256K" "512K" "1M" "2M" "4M" "8M")
	block_size_index="0"
		...
		do
			...
			do
				...
				do
					...
					then
						per_file_size="2048" # limit the max file size for a single process to 2G
					fi
					size=${per_file_size}M

					fio -filename=$testpath -iodepth 1 -thread -rw=${test_mode[mode_index]} -bs=${test_block_size[block_size_index]} -size=$size -numjobs=${num_jobs[num_jobs_index]} -name=mytest > $data_file_name

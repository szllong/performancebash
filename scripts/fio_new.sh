...
do
	...
	while ((size < end_size))
	do
		data_file_name=fio`echo $filename | sed 's/\//-/g'`-${test_mode[mode_index]}-bs$size
		
		fio -filename=$filename -iodepth 1 -thread -rw=${test_mode[mode_index]} -bs=$size -size=1G -numjobs=1 -name=mytest > $data_file_name
		>$size
		cp $data_file_name $size
